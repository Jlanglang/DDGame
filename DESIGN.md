...
**挑战模式 idle 惩罚**：随机选中一对已匹配牌开始倒计时；超时则盖回该对并重新随机目标对。惩罚秒数随关卡递减（第 1 关 12 秒 → 第 10 关 7 秒）。

### 4.3 关卡与对数

- 单关最多 **20 对**（40 张牌，布局算法上限）  
- **普通 3 关 · 困难 8 关 · 挑战 10 关**，对数与限制随关卡递增  
...
|------|---|---|-----|------------|
| 普通 / 每日 | 90% | 10% | 0% | 固定 |
| 困难 | 余量 | 9% | 1%→8% | 每关 SS +1%（第 1 关 1%，第 8 关 8%） |
| 挑战 | 余量 | 16% | 4%→13% | 每关 SS +1%（第 1 关 4%，第 10 关 13%） |
